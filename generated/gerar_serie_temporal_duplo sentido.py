...
try:
    ...
    # Cria listas com os horários em minutos a partir da meia-noite para ambos os sentidos
    minutos_desde_meia_noite_sentido_0 = [(dt - datas_horas_abertura_sentido_0[0]).total_seconds() / 60 for dt in datas_horas_abertura_sentido_0]
    minutos_desde_meia_noite_sentido_1 = [(dt - datas_horas_abertura_sentido_1[0]).total_seconds() / 60 for dt in datas_horas_abertura_sentido_1]

    # Cria o gráfico de série temporal com ambos os sentidos
    plt.figure(figsize=(12, 6))
    plt.plot(minutos_desde_meia_noite_sentido_1, range(len(minutos_desde_meia_noite_sentido_1)), label='Ida', color='red')
    plt.plot(minutos_desde_meia_noite_sentido_0, range(len(minutos_desde_meia_noite_sentido_0)), label='Volta', color='blue')

    plt.xlabel('Tempo (minutos desde a primeira viagem)')
    plt.ylabel('Número de Viagens')
    plt.title('Viagens por Sentido')
    plt.grid(True)
    plt.legend()
